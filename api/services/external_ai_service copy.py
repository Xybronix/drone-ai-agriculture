...
class ExternalAIService:
    ...
    async def identify_plant(self, image: Image.Image) -> Dict[str, Any]:
        """
    ...
    async def assess_health(self, image: Image.Image) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                logger.error(f"Plant.id Health API error: {response.status_code}")
                return self._error_response(f"API error: {response.status_code}")
    ...
    async def analyze_complete(self, image: Image.Image) -> Dict[str, Any]:
        """
        ...
        identification_task = self.identify_plant(image)
        health_task = self.assess_health(image)
